...
export async function handler(event) {
  try {
    ...
    const { url, fechaVencimientos, urlTerminos, destinatario, contador } = validacionCuerpo.datos;
    console.log("validacionCuerpo", validacionCuerpo);
    ...
    const command = new SendEmailCommand({
      ...
      ConfigurationSetName: "Enersa-SesConfigSet",
    });
    ...
    const responseDynamoDB = await ddbClient.send(commandDynamoDB);

    console.log("responseDynamoDB", responseDynamoDB);
